...
// ─── Canvas Cache ─────────────────────────────────────────────────────────────
// Single source of truth for canvas dimensions, refreshed on resize and on
// every frame that needs it. Avoids repeated getBoundingClientRect() calls.
...
// ─── Player ───────────────────────────────────────────────────────────────────
// Player.x / Player.y are canvas-relative coordinates of the element's
// top-left corner, matching the same space enemies and bullets use.

const Player = {
    x:      0,   // canvas-relative, top-left
    y:      0,   // updated by updatePlayerLayout()
    width:  0,
    height: 0,
    speed:  5,
};

/** Sync Player dimensions and Y from the DOM, then clamp X to canvas bounds. */
function updatePlayerLayout() {
    refreshCanvasRect();
    const r    = playerEl.getBoundingClientRect();
    Player.width  = r.width;
    Player.height = r.height;
    console.log("player x:", r.x);
    
    // Keep the stored canvas-relative Y current after resize
    Player.y = r.top - canvas.y;
    // Re-clamp X so the player doesn't leave the canvas after a resize
    if(Player.x==0) Player.x = r.x - canvas.x;
...
function applyPlayerTransform() {
    // Player is absolutely positioned; left edge at Player.x
    playerEl.style.left = `${Player.x}px`;
    playerEl.style.transform = 'none'; // drop the old translateX(-50%) trick
}
...
window.addEventListener('keydown', e => {
    ...
    if (e.key.toLowerCase() === 'p' && !gameState.ended) {
...
function updateLives() {
    let h = '';
    for (let i = 0; i < 3; i++) h += i < gameState.lives ? '♥ ' : '♡ ';
...
const shootBullet = throttle(() => {
    const bw = 8;
    const bh = 20;
    const x  = Player.x + Player.width / 2 - bw / 2;
    const y  = Player.y - bh;

    const el = document.createElement('img');
    el.src       = 'source/bullet_up.svg';
    el.alt       = '';
    el.className = 'bullet';
    el.style.left = `${x}px`;
    el.style.top  = `${y}px`;
    ...
    bullets.push({ element: el, x, y, width: bw, height: bh });
}, 500);

const fireEnemyBullet = throttle(() => {
    const alive = enemies.filter(e => e.alive);
    if (!alive.length) return;

    const shooter = alive[Math.floor(Math.random() * alive.length)];
    const x = shooter.x + ENEMY_WIDTH  / 2 - 4; // centre the 8px bullet
    const y = shooter.y + ENEMY_HEIGHT;

    const el = document.createElement('img');
    el.src       = 'source/bullet_down.svg';
    el.alt       = '';
    el.className = 'bullet';
    el.style.left = `${x}px`;
    el.style.top  = `${y}px`;
    ...
    enemyBullets.push({ element: el, x, y, width: 8, height: 20 });
}, 2000);

/**
 * Moves an array of bullets vertically and removes out-of-bounds ones.
 * @param {object[]} pool   - bullets or enemyBullets
 * @param {number}   dy     - pixels per frame (+down, -up)
 */
function moveBullets(pool, dy) {
    for (let i = pool.length - 1; i >= 0; i--) {
        const b = pool[i];
        b.y += dy;
        b.element.style.top = `${b.y}px`;

        const outOfBounds = dy < 0 ? b.y + b.height < 0
                                   : b.y > canvas.height;
        if (outOfBounds) {
            b.element.remove();
            pool.splice(i, 1);
...
function fireBullet() {
    ...
    moveBullets(bullets, -BULLET_SPEED);
}
...
function createEnemy(x, y) {
    ...
    el.style.left = `${x}px`;
    el.style.top  = `${y}px`;
    el.style.position = 'absolute'; // ensure it's positioned inside the canvas
    gameCanvas.appendChild(el);

    return { element: el, x, y, alive: true, width: ENEMY_WIDTH, height: ENEMY_HEIGHT };
}
...
function createEnemies() {
    ...
    for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
            enemies.push(createEnemy(startX + col * spacing, 30 + row * 50));
        }
    }
}

/** After a resize, pull any out-of-bounds enemies back inside the canvas. */
function clampEnemiestoCanvas() {
    for (const e of enemies) {
        if (!e.alive) continue;
        e.x = Math.max(0, Math.min(e.x, canvas.width - ENEMY_WIDTH));
        e.y = Math.max(0, Math.min(e.y, canvas.height - ENEMY_HEIGHT));
...
function moveEnemies() {
    if (getLowestEnemyY() + ENEMY_HEIGHT >= Player.y) {
        toggleControl('lose');
        return;
    }
    ...
    for (const e of enemies) {
        if (!e.alive) continue;
        e.x += ENEMY_SPEED * enemyDirection;
        applyEnemyTransform(e);
    }

    const { minX, maxX } = getEnemyXLimits();
    if (minX <= 0 || maxX >= canvas.width - ENEMY_WIDTH) {
        enemyDirection *= -1;
        for (const e of enemies) {
            if (!e.alive) continue;
            e.y += ENEMY_DROP;
...
function checkCollisions() {
    ...
    for (let i = enemyBullets.length - 1; i >= 0; i--) {
        if (!isColliding(enemyBullets[i], playerBox)) continue;
        enemyBullets[i].element.remove();
        enemyBullets.splice(i, 1);
        gameState.lives--;
        updateLives();
        if (gameState.lives <= 0) toggleControl('lose');
    }

    // Player bullets → enemies
    for (let j = bullets.length - 1; j >= 0; j--) {
        for (let i = enemies.length - 1; i >= 0; i--) {
            if (!isColliding(bullets[j], enemies[i])) continue;
            ...
            gameState.score += 20;
            updateScore();
            if (enemies.length === 0) toggleControl('win');
            break;
        }
    }
}
...
function toggleControl(state) {
    ...
    switch (state) {
        case 'paused': showMessage('Game Paused ⏸️', state); break;
        case 'win':
...
function showMessage(message, state) {
    ...
    controlEl.appendChild(title);

    if (state === 'paused') {
        controlEl.appendChild(resumeBtn);
    } else {
        const status = document.createElement('p');
        const m = String(Math.floor(gameState.time / 60)).padStart(2, '0');
        const s = String(Math.floor(gameState.time % 60)).padStart(2, '0');
        status.innerHTML = `Score: ${gameState.score}<br>Time: ${m}:${s}`;
...
function restart() {
    Object.assign(gameState, { score: 0, time: 0, lives: 3, paused: false, ended: false });
    updateLives();
    updateScore();

    [...bullets, ...enemyBullets, ...enemies].forEach(o => o.element.remove());
...
function gameLoop(timestamp) {
    ...
    if (!gameState.paused) {
        ...
        moveEnemies();
        fireEnemyBullet();
        moveBullets(enemyBullets, BULLET_SPEED);
        fireBullet();
        movePlayer();
    }
